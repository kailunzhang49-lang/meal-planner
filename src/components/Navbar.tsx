import { motion } from 'framer-motion'
import { NavLink } from 'react-router-dom'
import { CalendarDays, Clock, Heart } from 'lucide-react'
import { cn } from '../lib/utils'

const links = [
  { to: '/', label: '日历', icon: CalendarDays },
  { to: '/history', label: '历史', icon: Clock },
  { to: '/favorites', label: '收藏', icon: Heart },
]

export function Navbar() {
  return (
    <motion.header
      initial={{ y: -20, opacity: 0 }}
      animate={{ y: 0, opacity: 1 }}
      transition={{ duration: 0.5, delay: 0.1 }}
      className="sticky top-0 z-40"
    >
      <div className="glass border-b border-warm-200/40">
        <div className="max-w-lg mx-auto px-5">
          <div className="flex items-center justify-between h-14">
            {/* Logo */}
            <NavLink to="/" className="flex items-center gap-2 group">
              <motion.span
                className="text-xl"
                whileHover={{ rotate: [0, -10, 10, -5, 0] }}
                transition={{ duration: 0.5 }}
              >
                🥗
              </motion.span>
              <span className="font-semibold text-warm-700 group-hover:text-sage-600 transition-colors">
                每日菜谱
              </span>
            </NavLink>

            {/* Nav links */}
            <nav className="flex items-center gap-0.5">
              {links.map((link) => (
                <NavLink
                  key={link.to}
                  to={link.to}
                  end={link.to === '/'}
                  className="relative px-3 py-1.5"
                >
                  {({ isActive }) => (
                    <div className="flex items-center gap-1.5">
                      <link.icon
                        size={16}
                        className={cn(
                          'transition-colors',
                          isActive ? 'text-sage-600' : 'text-warm-400',
                        )}
                      />
                      <span
                        className={cn(
                          'text-sm font-medium transition-colors',
                          isActive ? 'text-sage-600' : 'text-warm-500',
                        )}
                      >
                        {link.label}
                      </span>
                      {isActive && (
                        <motion.div
                          layoutId="navbar-indicator"
                          className="absolute inset-0 bg-sage-100/60 rounded-lg -z-10"
                          transition={{ type: 'spring', stiffness: 380, damping: 30 }}
                        />
                      )}
                    </div>
                  )}
                </NavLink>
              ))}
            </nav>
          </div>
        </div>
      </div>
    </motion.header>
  )
}
